...
if ( isset($_POST['s2_admin']) && 'mail' == $_POST['s2_admin'] ) {
	...
	$status = $this->mail($recipients, $subject, $body, 'html');
	if ( $status ) {
...
echo "<div id=\"poststuff\" style=\"width: 60%;\" >";
global $wp_version;
if ( version_compare($this->wp_release, '3.3', '<') ) {
	the_editor($body, 'content', 'title', false);
} else {
	wp_editor($body, 'content', array( 'media_buttons' => false ));
}
echo "</div>";
echo "<br /><br />\r\n";
