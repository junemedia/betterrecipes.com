...
//	public function __construct( $content, $link = null, $timestamp, $twit, $twit_link, $twit_pic ) {
	...
	protected function linkify_content() {
		...
		$this->content = preg_replace( $hashtag_regex, '\1<a target="_blank" href="http://www.twitter.com/search?q=%23\2">#\2</a>', $this->content );
		...
		$this->content = preg_replace( $username_regex, '\1<a target="_blank" href="http://twitter.com/\2">@\2</a>', $this->content );
...
function _tt_make_web_ftp_clickable_cb( $matches ) {
	...
	return $matches[1] . "<a target='_blank' href=\"$dest\">$dest</a>$ret";
}
...
function _tt_make_url_clickable_cb($matches) {
	...
	return $matches[1] . "<a target='_blank' href=\"$url\">$url</a>" . $suffix;
}
