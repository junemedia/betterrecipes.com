...
class TwitterSearch
{
	...
	protected function do_search()
	{
		...
		foreach ( $this->feed->get_items() as $item ) {
			...
			$content = preg_replace( '/(^|[^a-z0-9_])@([a-z0-9_]+)/i', '$1<a target="_blank" href="http://twitter.com/$2">@$2</a>', $content );
			// En-link-ify all the hashtags in the content
			$content = preg_replace( '/(^|\s)#(\w*[a-z_]+\w*)/i', '\1<a target="_blank" href="http://www.twitter.com/search?q=%23\2">#\2</a>', $content );
