...
class GSliderPlugin {
	...
	function initialize_options() {
		...
		$this -> add_option('height', "auto");
		$this -> add_option('lightbox', "Y");
		$this -> add_option('affect', "slide");
		$this -> add_option('axis', "x");
		$this -> add_option('direction', "f");
		$this -> add_option('navigator', "false");
		$this -> add_option('lightbox_temp', "Y");
		$this -> add_option('autoslide', "Y");
		$this -> add_option('autoslide_temp', "Y");
		$this -> add_option('autospeed', 5000);
...
//	IF ( GS_LOAD_CSS )
	function gs_enqueue_styles() {
		global $version;
		$gsliderStyleUrl = GS_PLUGIN_URL . '/css/gslider-css.php?v='. $version .'&amp;pID=' . $GLOBALS['post']->ID;
        $gsliderStyleFile = GS_PLUGIN_DIR . '/css/gslider-css.php';
//		$src = WP_PLUGIN_DIR.'/' . $this -> plugin_name . '/css/gslider-css.php?2=1&site='.WP_PLUGIN_DIR;
//		define $infogal = $this;
		$infogal = $this;
		if ( file_exists($gsliderStyleFile) ) {
			if ($styles = $this -> get_option('styles')) {
				foreach ($styles as $skey => $sval) {
					$gsliderStyleUrl .= "&amp;" . $skey . "=" . urlencode($sval);
				}
			}
			if ($width_temp = $this-> get_option('width_temp')) {
				foreach ($width_temp as $skey => $sval) {
					if ($skey == $GLOBALS['post']->ID)
					$gsliderStyleUrl .= "&amp;width_temp=" . urlencode($sval);
				}
			}
			if ($height_temp = $this-> get_option('height_temp')) {
				foreach ($height_temp as $skey => $sval) {
					if ($skey == $GLOBALS['post']->ID)
					$gsliderStyleUrl .= "&amp;height_temp=" . urlencode($sval);
				}
			}
			wp_register_style( 'gallery-slider', $gsliderStyleUrl);
			wp_enqueue_style( 'gallery-slider', $gsliderStyleUrl,	array(), GS_VERSION, 'all' );
		}
	...
	function enqueue_scripts() {	
		...
		if (is_admin()) {
			...
			if (!empty($_GET['page'])) {
				...
				if ($_GET['page'] == "gslider-slides" && $_GET['method'] == "order") {
			...
			wp_enqueue_script($this -> plugin_name, '/' . PLUGINDIR . '/' . $this -> plugin_name . '/js/jquery.ulslide.js', null, '1.0');
			wp_enqueue_script($this -> plugin_name.'-preload', '/' . PLUGINDIR . '/' . $this -> plugin_name . '/js/preload.js', null, '1.0');
