...
class GSliderSlide extends GSliderDbHelper {
	...
	var $plugin_name = GS_PLUGIN_NAME;
	...
	function GSliderSlide($data = array()) {
	...
	function defaults() {
		...
		$defaults = array(
			...
			'created'			=>	GSliderHtmlHelper::gen_date(),



			'modified'			=>	GSliderHtmlHelper::gen_date(),
		...
		if (!empty($data)) {
			...
			if (empty($image_url)) { $this -> errors['image_url'] = __('Please specify an image', $this -> plugin_name); }
				...
				if ($image = wp_remote_fopen($image_url)) {
					...
					$filepath = ABSPATH . 'wp-content' . DS . 'uploads' . DS . GS_PLUGIN_NAME . DS;
					...
					if (!file_exists($filefull)) {
						...
						$name = GSliderHtmlHelper::strip_ext($filename, 'filename');



						$ext = GSliderHtmlHelper::strip_ext($filename, 'ext');
