...
class GSliderPaginate extends GSliderPlugin {
	...
	function GSliderPaginate($table = '', $fields = '', $sub = '', $parent = '') {
	...
	function start_paging($page = '') {
		...
		if (count($records) < $allRecordsCount) {			
			...
			if ($this -> page > 1) {
				...
				$this -> pagination .= '<a class="prev page-numbers" href="' . GSliderHtmlHelper::retainquery($add_prev) . '" title="' . __('Previous Page', $this -> plugin_name) . '">&laquo;</a>';
			...
			while ($p <= $allRecordsCount) {
				...
				if ($k >= ($this -> page - 5) && $k <= ($this -> page + 5)) {
					...
					if ($k != $this -> page) {
						...
						$this -> pagination .= '<a class="page-numbers" href="' . GSliderHtmlHelper::retainquery($add_numbers) . '" title="' . __('Page', $this -> plugin_name) . ' ' . $k . '">' . $k . '</a>';
			...
			if ((count($records) + $this -> begRecord) < $allRecordsCount) {
				...
				$this -> pagination .= '<a class="next page-numbers" href="' . GSliderHtmlHelper::retainquery($add_next) . '" title="' . __('Next Page', $this -> plugin_name) . '">&raquo;</a>';
