...
Description: Enables you to give a link to anonymous users for public preview of a post before it is published. This has been modified to use the siteurl option vs. the home option. Creating an URL that is previewable under Reverse Proxy.
Author: Matt Martz</a> and <a href='http://www.ginside.com/'>Jonathan Dingman</a>, Modified by Wade Couch.
Version: 1.3
...
<?php
			if ( ! isset($preview_posts[$post->ID]) ) {
				...
				$url = htmlentities(add_query_arg(array('p' => $this->id, 'preview' => 'true', 'preview_id' => $this->id, 'public' => true, 'nonce' => $nonce), get_option('siteurl') . '/'));
				// Wade Couch - changed home to siteurl

				echo "<p><a href='$url' target='_blank'>$url</a><br /><br />\r\n";
				// Wade Couch - Added target=_blank
			}
		} else {
