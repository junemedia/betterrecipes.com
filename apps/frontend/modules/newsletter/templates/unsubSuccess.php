<div id="main-content">
                  <div id="theme-wrap">
                    <div class="wrapper">
                      <div class="section" style="min-height:550px;">
                        <div class="article">
                            <div id="pagebody">
                            <div id="singlecolumn"><div id="singlecolumnwell">
<br>
<table width="650" cellpadding="0" cellspacing="0" border="0" class="quickSignupContainer">
    <tr>
        <td width="50" class="quickNewsHide">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td><img src="/img/img-newsletter/br_regtopper_weeklyNL.jpg"></td>
    </tr>
</table>
<br>
<table width="650" cellpadding="0" cellspacing="0" border="0" class="quickSignupContainer">
    <form id="quicksignForm" action="" method="post" onsubmit="return check_fields(this)">
        <tr>
            <td width="50" class="quickNewsHide">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
            <td>
                <br><br>

                <table width="600" cellpadding="0" cellspacing="0" border="0" class="quickNewsUserInfo">
				<tr><td style="height:10px;"></td></tr>
        <tr><td style="color:#000;">
    <iframe width="575" height="700" frameborder="0" src="http://r4l.popularliving.com/subctr/unsub/br/entry.php?listid=<?php echo $lid;?>&amp;jobid=<?php echo $jid?>&amp;email=<?php echo $email;?>" id="new_unsubscribe" scrolling="No"></iframe>
    </td></tr>

                </table>
                <!-- End info table -->
            </td>
        </tr></form></table><br><br><br>

</div>
            <div class="clearall"></div>
		</div>
    </div>
                        </div><!-- /.article -->
                        <div class="sidebar">
                        </div><!-- /.sidebar -->
                        <div class="clearfix"></div>
                      </div><!-- /.section -->
                    </div><!-- /.wrapper -->
                  </div><!-- /#theme-wrap -->
                </div><!-- /#main-content -->
   
