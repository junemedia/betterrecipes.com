...
class GSRequest {
  ...
  public function __construct($apiKey, $secretKey, $apiMethod, $params = null, $useHTTPS = false) {
    ...
    if (strrpos($apiMethod, ".") == 0) {
      ...
      $this->domain = "socialize.resolute.com";
...
class GSDictionary {
  ...
  public function __construct($json=null) {
    ...
    if (!empty($json)) {
      ...
      if (!empty($json) && gettype($json) == 'string') {
  ...
  public function serialize() {
    ...
    $arr = $this->serializeGSDictionary($this);

    return $arr;
  }

  function serializeGSDictionary($gsd) {
    $arr = Array();
    foreach ($gsd->map as $name => $value) {

      $val = $this->serializeValue($value);
      $arr[$name] = $val;
    }
    return $arr;
  }
  ...
  function serializeValue($value) {
    //GSDictionary
    if ($value instanceof GSDictionary) {
      return $this->serializeGSDictionary($value);
    }
    ...
    //array
      ...
      foreach ($value as $key => $val) {
        $gsv = $this->serializeGSDictionary($val);
        $childArray[$key] = $gsv;
      }
  ...
  public function toJsonString() {
    try {
      return json_encode($this->serialize($this));
    } catch (Exception $e) {
      return null;
    }
  }
